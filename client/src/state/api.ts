...
export const api = createApi({
  ...
  endpoints: (build) => ({
    getAuthUser: build.query<User, void>({
      queryFn: async (_, _queryApi, _extraoptions, fetchWithBQ) => {
        try {
          ...
          const endpoints = userRole === "manager" ? `/managers/${user.userId}` : `/tenants/${user.userId}`;
    ...
    >({
      query: (filters) => {
        const params = cleanParams({
          ...
          availableFrom: filters.availableForm,
    ...
    updateApplicationStatus: build.mutation<Application & { lease?: Lease },
