...
export const FilterFull = () => {
    ...
    const handleAmenityChange = (amentity: AmenityEnum) => {
        setLocalFilters((prev) => ({
            ...prev,
            amenities: prev.amenities.includes(amentity)
                ? prev.amenities.filter((a) => a !== amentity)
                : [...prev.amenities, amentity]
        }))
    };
    ...
    const handleLocationSearch = async () => {
        try {
            const response = await fetch(
                `htts://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
            ...
            if (data.feature && data.feature.length > 0) {
                const [lng , lat] = data.feature[0].center;
                setLocalFilters((prev) => ({
                    ...prev,
                    coordinates: [lng, lat]
                }))
            ...
            <div className="flex flex-col space-y-6">
                ...
                <div>
                    ...
                    <Input
                        type="date"
                        value={
                            localFilters.availableForm !== "any"
                                ? localFilters.availableForm
                                : ""
                        }
                        onChange={(e) =>
                            setLocalFilters((prev) => ({
